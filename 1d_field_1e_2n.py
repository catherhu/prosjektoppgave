...
for i in range(len(x)):
    V[i] = -1/T * (integrate.trapezoid(2 / np.sqrt((x[i] - R + a_0 * np.cos(t)) ** 2 + alpha), x = t)
                   + integrate.trapezoid(2 / np.sqrt((x[i] + R + a_0 * np.cos(t)) ** 2 + alpha), x = t))
...
plt.legend()
plt.savefig("1d_field_1e_2n.png")